...
void before_linuxpu_load_module_path(hook_fargs4_t* args, void* udata) {
...
void before_linuxpu_unload_module(hook_fargs3_t* args,void* udata) {
...
void before_linuxpu_kpm_num(hook_fargs1_t* args, void* udata) {
...
void before_linuxpu_kpm_list(hook_fargs3_t* args, void* udata) {
...
void before_linuxpu_kpm_info(hook_fargs3_t* args, void* udata) {
...
void before_linuxpu_kpm_version(hook_fargs3_t* args, void* udata) {
    char __user* out = (char __user*) args->arg0;
...
void before_linuxpu_kpm_control(hook_fargs3_t* args, void* udata) {
...
void init_linuxpu() {
    ...
    addr = kallsyms_lookup_name("linuxpu_kpm_load_module_path");
    if(addr) {
        rc = hook_wrap4((void*) addr, before_linuxpu_load_module_path, NULL, NULL);
        log_boot("hook linuxpu_load_module_path rc:%d \n", rc);
    } else {
        log_boot("hook linuxpu_load_module_path faild \n", rc);
    }

    addr = kallsyms_lookup_name("linuxpu_kpm_unload_module");
    if(addr) {
        rc = hook_wrap3((void*) addr, before_linuxpu_unload_module, NULL, NULL);
        log_boot("hook linuxpu_kpm_unload_module rc:%d \n", rc);
    } else {
        log_boot("hook linuxpu_kpm_unload_module faild \n", rc);
    }

    addr = kallsyms_lookup_name("linuxpu_kpm_num");
    if(addr) {
        rc = hook_wrap1((void*) addr, before_linuxpu_kpm_num, NULL, NULL);
        log_boot("hook linuxpu_kpm_num rc:%d \n", rc);
    } else {
        log_boot("hook linuxpu_kpm_num faild \n", rc);
    }

    addr = kallsyms_lookup_name("linuxpu_kpm_list");
    if(addr) {
        rc = hook_wrap3((void*) addr, before_linuxpu_kpm_list, NULL, NULL);
        log_boot("hook linuxpu_kpm_list rc:%d \n", rc);
    } else {
        log_boot("hook linuxpu_kpm_list faild \n", rc);
    }

    addr = kallsyms_lookup_name("linuxpu_kpm_info");
    if(addr) {
        rc = hook_wrap3((void*) addr, before_linuxpu_kpm_info, NULL, NULL);
        log_boot("hook linuxpu_kpm_info rc:%d \n", rc);
    } else {
        log_boot("hook linuxpu_kpm_info faild \n", rc);
    }

    addr = kallsyms_lookup_name("linuxpu_kpm_control");
    if(addr) {
        rc = hook_wrap3((void*) addr, before_linuxpu_kpm_control, NULL, NULL);
        log_boot("hook linuxpu_kpm_control rc:%d \n", rc);
    } else {
        log_boot("hook linuxpu_kpm_control faild \n", rc);
    }

    addr = kallsyms_lookup_name("linuxpu_kpm_version");
    if(addr) {
        rc = hook_wrap3((void*) addr, before_linuxpu_kpm_version, NULL, NULL);
        log_boot("hook linuxpu_kpm_version rc:%d \n", rc);
    } else {
        log_boot("hook linuxpu_kpm_version faild \n", rc);
    }
